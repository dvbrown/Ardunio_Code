...
int setHour = 6; // Set hours to wake (military time)
int setMin = 34; // Set minute to wake
...
unsigned char minute = 66;
unsigned char hour = 06;
...
void setup()
{
  ...
  Timer1.initialize(5000);//timing for 5ms
...
void TimingISR()
{
  halfsecond ++;
  Update = ON;
  if(halfsecond == 200){ // Changed from 2 to 200
    second ++;
    if(second == 60)
    {
      minute ++;
